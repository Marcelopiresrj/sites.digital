    ...
    try {
        ...
        for (const ag of agendamentos) {
            ...
            if (diffMinutes <= 30 && diffMinutes > 0 && !ag.lembrete_30m_enviado) {
                await MegaApi.sendMessage(clienteTel, `⏳ Olá ${cliente.nome}! Passando para lembrar que seu agendamento de *${servico.nome}* é em cerca de 30 minutos (às ${horaStr}). Até logo!`);
            ...
            // Regra 60 min (entre 30 e 60 minutos)
                ...
                await MegaApi.sendMessage(clienteTel, `⏳ Olá ${cliente.nome}! Passando para lembrar que seu agendamento de *${servico.nome}* é em cerca de 1 hora (às ${horaStr}). Até logo!`);
            ...
            // Regra 24 horas (entre 23h e 24h = 1380 a 1440 minutos)
                ...
                await MegaApi.sendMessage(clienteTel, `📅 Olá ${cliente.nome}! Lembrete: você tem um agendamento de *${servico.nome}* amanhã (${dataStr}) às ${horaStr}.`);
